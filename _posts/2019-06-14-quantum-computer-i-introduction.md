...
量子電腦與我們使用的經典電腦有著很大的不同。在經典電腦中，我們使用 bit 也就是常聽到的 1 和 0 來做計算單位。在量子電腦裡，最小的計算單位是 Qubit (量子比特)，它有著一些超乎常理的特性例如疊加態，糾纏態，使量子電腦可以在某些問題的計算上比經典電腦還要快。

量子電腦的出現並不會取代經典電腦。它們之間的關係比較像是車子與船的差別，各自有自己擅長解決的領域。因此可以期待在未來看到兩種電腦並行一起解決各種難題。
